let closeErrorIcon = document.querySelector(".close-error ");
...
// close the error event
closeErrorIcon.onclick = function (e) {
  e.preventDefault();
  error.classList.add("hide-display");
};

// github data object, Add your token
const githubData = {
  token: "[git token]",
  username: "",
};
...
signOut.onclick = function(e) {
  e.preventDefault();
  authSection.classList.remove("hide-display");
  profileSection.classList.add("hide-display");
...
form.addEventListener("submit", function (e) {
  ...
  // assign input to username
  let input = document.querySelector(".search").value;
  githubData["username"] = input;
...
query {
  ...
  `;
    ...
    const renderUserData = ({ data: { user } }) => {
      // nav user images
      document.getElementById("profile-icon").src = user.avatarUrl;
      document.getElementById("profile-icons").src = user.avatarUrl;
      let profileNav = document.getElementById("nav-profile")
    ...
    <div class="profile-picture">
      ...
      ${
        ...
        <span class="status-icon">
          ${user.status.emojiHTML || '<img src="https://github.githubassets.com/images/icons/emoji/unicode/1f4ad.png" alt="icon">'}
        </span>
  ...
  <section class="profile-details dflex flex-col align-start full-width mb">
    ...
    const renderRepos = ({
      ...
      <div class="repo-details dflex flex-col align-start">
        <h3 class="repo-title dflex justify-start">
          <a href="https://github.com/${githubData.username}/${node.name}" target="_blank">${node.name}</a>
          ${
            node.isPrivate
              ? `<span class="badge dflex ml-0-4">Private</span>`
              : ""
          }
        </h3>
        ${
          node.shortDescriptionHTML
            ? `<p class="repo-description small-text mt-0-8">${node.shortDescriptionHTML}</p>`
            : ""
        }
    ...
    myHeaders.append("Authorization", `Bearer ${githubData["token"]}`);
    myHeaders.append("Content-Type", "application/json");

    var graphql = JSON.stringify({
      query: getRepos(githubData["username"]),
      variables: {},
    });
    var requestOptions = {
      method: "POST",
    ...
    fetch("https://api.github.com/graphql", requestOptions)
      ...
      .then((res) => {
        ...
        if (res.data["user"] === null) {
          error.classList.remove("hide-display");
          return;
        }
        ...
        authSection.classList.add("hide-display");
        profileSection.classList.remove("hide-display");
      })
